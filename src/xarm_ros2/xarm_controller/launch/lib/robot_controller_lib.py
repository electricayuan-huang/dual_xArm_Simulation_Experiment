...
def configure_joint_state_broadcaster(ros2_control_params, joints):
    """Publish only canonical joint names understood by robot_state_publisher/MoveIt.

    gz_ros2_control exposes native mimic states with an internal ``_mimic``
    suffix.  Those states are useful to the simulator, but are not joints in
    the URDF consumed by MoveIt.  Broadcasting the active joints is sufficient
    because both consumers reconstruct the mimic joints from the URDF.
    """
...
def generate_ros2_control_params_temp_file(ros2_control_params_path, prefix='', add_gripper=False, add_bio_gripper=False, ros_namespace='', update_rate=None, robot_type='xarm'):
    if ros_namespace or prefix or add_gripper or add_bio_gripper or update_rate:
        ...
        if add_gripper:
            joint_names.append('{}drive_joint'.format(prefix))
...
def generate_dual_ros2_control_params_temp_file(
    ...
    if add_gripper_1:
        joint_names.append('{}drive_joint'.format(prefix_1))
    if add_gripper_2:
        joint_names.append('{}drive_joint'.format(prefix_2))
    configure_joint_state_broadcaster(ros2_control_params_yaml, joint_names)
    if ros_namespace:
        ros2_control_params_yaml = {
            ros_namespace: ros2_control_params_yaml
