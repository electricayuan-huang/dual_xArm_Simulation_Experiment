...
def add_prefix_to_ros2_control_params(prefix, ros2_control_params):
    ...
    for name in list(ros2_control_params.keys()):
        ...
        gains = ros__parameters.get('gains', {})
        constraints = ros__parameters.get('constraints', {})
        initial_positions = ros__parameters.get('initial_positions', {})
        ...
        for joint in list(initial_positions.keys()):
            initial_positions['{}{}'.format(prefix, joint)] = initial_positions.pop(joint)
        for joint in list(gains.keys()):
            if not joint.startswith(prefix):
                gains['{}{}'.format(prefix, joint)] = gains.pop(joint)
...
def configure_joint_state_broadcaster(ros2_control_params, joints):
    """Publish only canonical joints understood by robot_state_publisher and MoveIt.

    gz_ros2_control may expose native mimic state names with a ``_mimic``
    suffix.  The URDF already reconstructs those mimic joints, so publishing
    the active joints only prevents invalid names from reaching MoveIt.
    """
    ros2_control_params['joint_state_broadcaster'] = {
        'ros__parameters': {
            'joints': list(joints),
...
def generate_ros2_control_params_temp_file(ros2_control_params_path, prefix='', add_gripper=False, add_bio_gripper=False, ros_namespace='', update_rate=None, robot_type='xarm', use_sim_time=False, initial_positions=None):
    if ros_namespace or prefix or add_gripper or add_bio_gripper or update_rate:
        ...
        if initial_positions:
            for controller in ros2_control_params_yaml.values():
                params = controller.get('ros__parameters', {}) if isinstance(controller, dict) else {}
                if 'initial_positions' in params:
                    params['initial_positions'].update(initial_positions)
                    break
                 
        add_prefix_to_ros2_control_params(prefix, ros2_control_params_yaml)
        joint_names = [
            '{}joint{}'.format(prefix, index) for index in range(1, 7)
        ]
        if add_gripper:
            joint_names.extend([
                '{}drive_joint'.format(prefix),
                '{}left_finger_joint'.format(prefix),
                '{}left_inner_knuckle_joint'.format(prefix),
                '{}right_outer_knuckle_joint'.format(prefix),
                '{}right_finger_joint'.format(prefix),
                '{}right_inner_knuckle_joint'.format(prefix),
            ])
        configure_joint_state_broadcaster(ros2_control_params_yaml, joint_names)
        if ros_namespace:
...
def generate_dual_ros2_control_params_temp_file(
    ...
    joint_names = [
        '{}joint{}'.format(prefix_1, index) for index in range(1, 7)
    ] + [
        '{}joint{}'.format(prefix_2, index) for index in range(1, 7)
    ]
    if add_gripper_1:
        joint_names.extend([
            '{}drive_joint'.format(prefix_1),
            '{}left_finger_joint'.format(prefix_1),
            '{}left_inner_knuckle_joint'.format(prefix_1),
            '{}right_outer_knuckle_joint'.format(prefix_1),
            '{}right_finger_joint'.format(prefix_1),
            '{}right_inner_knuckle_joint'.format(prefix_1),
        ])
    if add_gripper_2:
        joint_names.extend([
            '{}drive_joint'.format(prefix_2),
            '{}left_finger_joint'.format(prefix_2),
            '{}left_inner_knuckle_joint'.format(prefix_2),
            '{}right_outer_knuckle_joint'.format(prefix_2),
            '{}right_finger_joint'.format(prefix_2),
            '{}right_inner_knuckle_joint'.format(prefix_2),
        ])
    configure_joint_state_broadcaster(ros2_control_params_yaml, joint_names)
    if ros_namespace:
        ros2_control_params_yaml = {
