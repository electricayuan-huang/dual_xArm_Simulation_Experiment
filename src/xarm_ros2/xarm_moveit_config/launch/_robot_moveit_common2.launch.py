...
def launch_setup(context, *args, **kwargs):
    ...
    if not rviz_config.perform(context):
        rviz_config_file = PathJoinSubstitution([
            FindPackageShare(moveit_config_package_name), 'rviz', 'xarm6_moveit_clean.rviz'
        ])
    else:
    ...
    xyz = attach_xyz.perform(context).strip('"').split()
    rpy = attach_rpy.perform(context).strip('"').split()
    # Use named arguments. The positional static_transform_publisher syntax
    # uses a different RPY order and previously rotated link_base incorrectly.
    args = [
        '--x', xyz[0], '--y', xyz[1], '--z', xyz[2],
        '--roll', rpy[0], '--pitch', rpy[1], '--yaw', rpy[2],
        '--frame-id', attach_to.perform(context),
        '--child-frame-id', '{}link_base'.format(prefix.perform(context)),
    ]
